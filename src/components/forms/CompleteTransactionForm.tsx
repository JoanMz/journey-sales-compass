...
const CompleteTransactionForm: React.FC<CompleteTransactionFormProps> = ({
  ...
  const validatePackageFields = (data: SalesFormData): string[] => {
    ...
    if (data.package === "nacional" || data.package === "internacional" || data.package === "migratorio") {
      if (!data.flightInfo || data.flightInfo.length === 0) {
        errors.push("• Al menos un vuelo es requerido");
      } else {
        const firstFlight = data.flightInfo[0];
        if (!firstFlight.aerolinea || !firstFlight.ruta || !firstFlight.hora_salida || !firstFlight.hora_llegada) {
          errors.push("• Información completa del vuelo (aerolínea, ruta, hora salida, hora llegada)");
        }
      }
    }

    // Validar información de hotel
    if (data.package === "nacional" || data.package === "internacional" || data.package === "terrestre") {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let hasFlightInfo = formData.flightInfo && formData.flightInfo.length > 0 && 
      ...
      let hasHotelInfo = formData.hotelInfo && formData.hotelInfo.length > 0 &&
      ...
      switch (formData.package) {
        case "migratorio":
          if (!hasFlightInfo) {
            validationMessage = "Para paquetes migratorios, debes completar al menos la información del vuelo.";
          }
          break;
        case "terrestre":
          if (!hasHotelInfo) {
            validationMessage = "Para paquetes terrestres, debes completar al menos la información del hotel.";
          }
          break;
        case "nacional":
        case "internacional":
          if (!hasFlightInfo && !hasHotelInfo) {
            validationMessage = "Para paquetes nacionales e internacionales, debes completar al menos la información del vuelo o del hotel.";
          }
          break;
      }
      ...
      try {
        ...
        console.log("✅ Campos disponibles en la transacción:", Object.keys(currentTransaction));
        console.log("✅ Estructura de itinerario:", currentTransaction.itinerario);
        console.log("✅ Estructura de travel_info:", currentTransaction.travel_info);
        console.log("✅ Estructura de flight_info:", currentTransaction.flight_info);
        console.log("✅ Estructura de hotel_info:", currentTransaction.hotel_info);
        
        // Verificar qué datos ya existían
        console.log("📋 Datos existentes antes de actualizar:");
        console.log("📋 Itinerario existente:", currentTransaction.itinerario?.length > 0 ? "SÍ" : "NO");
        console.log("📋 Travel info existente:", currentTransaction.travel_info?.length > 0 ? "SÍ" : "NO");
      } catch (getError) {
        console.error("❌ Error obteniendo transacción actual:", getError.response?.status, getError.response?.data);
        alert(`Error: No se pudo obtener la transacción ${transactionId}. Verifica que el ID sea correcto.`);
        setIsSubmitting(false);
        return;
      }
      ...
      try {
        ...
        const hasFlightInfo = formData.flightInfo && formData.flightInfo.length > 0 && 
          (formData.flightInfo[0]?.aerolinea?.trim() || 
           formData.flightInfo[0]?.ruta?.trim() || 
           formData.flightInfo[0]?.hora_salida?.trim() || 
           formData.flightInfo[0]?.hora_llegada?.trim());
        ...
        const hasHotelInfo = formData.hotelInfo && formData.hotelInfo.length > 0 && 
          (formData.hotelInfo[0]?.hotel?.trim() || 
           formData.hotelInfo[0]?.noches > 0);
        ...
        if (hasFlightInfo && (formData.package === "migratorio" || formData.package === "nacional" || formData.package === "internacional")) {
          console.log("📤 Actualizando itinerario (requerido para este tipo de paquete)...");
          
          try {
            // Verificar si ya existe un itinerario
            console.log("🔍 Itinerario actual en la transacción:", currentTransaction.itinerario);
            
            if (currentTransaction.itinerario && currentTransaction.itinerario.length > 0) {
              // Actualizar itinerario existente
              const itinerarioId = currentTransaction.itinerario[0].id;
              console.log("📤 Actualizando itinerario existente con ID:", itinerarioId);
              console.log("📤 Itinerario completo:", currentTransaction.itinerario[0]);
              
              console.log("📤 URL del endpoint itinerario:", `https://fastapi-data-1-nc7j.onrender.com/transactions/${transactionId}/itinerario/${itinerarioId}`);
              console.log("📤 Payload itinerario:", JSON.stringify(formData.flightInfo[0], null, 2));
              console.log("📤 Headers:", {
              ...
              console.log("✅ Itinerario actualizado exitosamente:", itinerarioResponse.data);
              ...
              console.log("✅ Nuevo itinerario creado exitosamente:", itinerarioResponse.data);
              itinerarioActualizado = true;
            }
            ...
            alert("⚠️ ADVERTENCIA: La información del vuelo no se pudo actualizar. Contacta al administrador para verificar el endpoint de itinerario.");
          }
        } else if (hasFlightInfo) {
          console.log("ℹ️ Información de vuelo disponible pero no requerida para paquete", formData.package);
        }

        // Actualizar travel_info solo si es necesario según el tipo de paquete
        if (hasHotelInfo && (formData.package === "terrestre" || formData.package === "nacional" || formData.package === "internacional")) {
          console.log("📤 Actualizando travel_info (requerido para este tipo de paquete)...");
          ...
          console.log("   - pais_destino:", formData.hotelInfo[0]?.pais_destino);
          console.log("   - ciudad_destino:", formData.hotelInfo[0]?.ciudad_destino);
          
          try {
            // Verificar si ya existe travel_info
            console.log("🔍 Travel info actual en la transacción:", currentTransaction.travel_info);
            
            if (currentTransaction.travel_info && currentTransaction.travel_info.length > 0) {
              // Actualizar travel_info existente
              const travelInfoId = currentTransaction.travel_info[0].id;
              console.log("📤 Actualizando travel_info existente con ID:", travelInfoId);
              console.log("📤 Travel info completo:", currentTransaction.travel_info[0]);
              
              console.log("📤 URL del endpoint travel_info:", `https://fastapi-data-1-nc7j.onrender.com/transactions/${transactionId}/travel_info/${travelInfoId}`);
              console.log("📤 Payload travel_info:", JSON.stringify(travelInfoPayload, null, 2));
              console.log("📤 Headers:", {
                "Content-Type": "application/json",
              });
              ...
              console.log("✅ Travel info actualizado exitosamente:", travelInfoResponse.data);
              ...
              console.log("✅ Nuevo travel_info creado exitosamente:", travelInfoResponse.data);
              travelInfoActualizado = true;
            }
          } catch (travelInfoError) {
            console.error("❌ Error actualizando travel_info:", travelInfoError);
            ...
            alert("⚠️ ADVERTENCIA: La información del hotel no se pudo actualizar. Revisa la consola para más detalles.");
          }
        } else if (hasHotelInfo) {
          console.log("ℹ️ Información de hotel disponible pero no requerida para paquete", formData.package);
        }
        ...
        console.log("✅ Datos actualizados en el backend:", verifyResponse.data);
        console.log("✅ Itinerario en backend:", verifyResponse.data.itinerario);
        console.log("✅ Travel info en backend:", verifyResponse.data.travel_info);
        
        // Mostrar resultado
        if (itinerarioActualizado || travelInfoActualizado) {
          console.log("✅ Los datos se actualizaron correctamente en el backend");
          const updatedFields = [];
          ...
          const successMessage = `✅ Información actualizada correctamente: ${updatedFields.join(" y ")}`;
          alert(successMessage);
        } else {
          console.log("ℹ️ No se actualizó ningún campo");
          alert("ℹ️ No se actualizó ningún campo. Verifica que hayas llenado al menos un campo.");
        }
              } catch (patchError) {
          ...
          if (patchError.response?.data?.detail) {
            ...
            patchError.response.data.detail.forEach((error: any, index: number) => {
      ...
      const travelersWithDocuments = formData.travelers.filter(traveler => traveler.dniImage);
      ...
      if (travelersWithDocuments.length > 0) {
        ...
        if (!formData.documentType || formData.documentType === "") {
          alert("Debes seleccionar un tipo de documento para los viajeros antes de continuar.");
          setIsSubmitting(false);
          return;
        }
        ...
        try {
          ...
          travelersWithDocuments.forEach((traveler, index) => {
            if (traveler.dniImage) {
              ...
              console.log(`✅ Agregando imagen del DNI del viajero ${index + 1}: ${traveler.name} (DNI: ${traveler.dni}) - ${traveler.dniImage.name}`);
            }
          });
          ...
          if (allImagesFormData.has("traveler_dni_0")) {
            ...
            if (imageResponse.data && imageResponse.data.length > 0) {
              ...
              console.log("URLs de los documentos obtenidas:", uploadedImageUrls);
            }
            ...
            for (let i = 0; i < travelersWithDocuments.length; i++) {
              ...
              if (documentUrl) {
                try {
                  ...
                  console.log(`📤 Enviando documento para viajero ${traveler.dni}...`);
                  ...
                  console.log(`✅ Documento enviado para viajero ${traveler.dni}:`, documentoResponse.data);
                  documentosExitosos++;
                  
                } catch (error) {
                  console.error(`❌ Error enviando documento para viajero ${traveler.dni}:`, error);
                  console.log("💡 Nota: El backend actual espera archivos binarios. Cambia el backend para recibir URLs.");
                }
              }
            }
            
            console.log(`📊 Resumen de documentos: ${documentosExitosos}/${travelersWithDocuments.length} enviados exitosamente`);
          }
        } catch (error) {
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
        ...
        <p className="text-blue-700">
          Tipo de paquete: <span className="font-semibold">{formData.package}</span>
        </p>
        <p className="text-sm text-blue-600 mt-2">
          Completa la información faltante según el tipo de paquete seleccionado.
        </p>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => updateHotelField(index, "noches", parseInt(e.target.value))}
                  min="1"
          ...
          <div className="space-y-4">
            <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
              <p className="text-sm text-yellow-700">
                <strong>Nota:</strong> Si los viajeros no tienen documentos subidos, puedes agregarlos aquí.
                Los documentos se enviarán automáticamente al backend después de completar la información.
              </p>
            </div>
      ...
      <div className="bg-white p-4 rounded-lg border">
        <div className="flex justify-between items-center mb-2">
          <span className="text-sm font-medium text-gray-700">Progreso de completado</span>
          <span className="text-sm text-gray-500">
            {isPackageCompleteForConfirmation(formData) ? "✅ Completo" : "⚠️ Incompleto"}
          </span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div 
            className={`h-2 rounded-full ${
              isPackageCompleteForConfirmation(formData) ? 'bg-green-500' : 'bg-yellow-500'
            }`}
            style={{ width: isPackageCompleteForConfirmation(formData) ? '100%' : '70%' }}
          ></div>
        </div>
      </div>
          ...
          onClick={async () => {
            try {
              ...
              if (validationErrors.length > 0) {
                alert(`❌ Validación fallida. Por favor completa los siguientes campos:\n${validationErrors.join('\n')}`);
                return; // Detiene todo el proceso aquí
              }
              
              console.log("✅ Validación exitosa - todos los campos requeridos están completos");
              ...
              const requiresFlight = ["nacional", "internacional", "migratorio"].includes(formData.package);
              if (requiresFlight && formData.flightInfo && formData.flightInfo.length > 0) {
                ...
                for (let i = 0; i < formData.flightInfo.length; i++) {
                  ...
                  const hasValidFlightData = flight.aerolinea && flight.aerolinea.trim() !== "" &&
                                           flight.ruta && flight.ruta.trim() !== "" &&
                                           flight.fecha && flight.hora_salida && flight.hora_llegada;
                  
                  if (!hasValidFlightData) {
                    console.log(`⏭️ Saltando vuelo ${i + 1} - datos incompletos`);
                    continue;
                  }
                  
                  // Verificar si este vuelo ya tiene ID (existe en el backend)
                  // Si el vuelo viene del currentData y tiene ID, es porque ya existe
                  const existingFlight = currentData.flightInfo && currentData.flightInfo[i];
                  
                  if (existingFlight && (existingFlight as any).id) {
                    // PATCH - Actualizar vuelo existente
                    console.log(`📤 Actualizando vuelo existente ID: ${(existingFlight as any).id}`);
                    await axios.patch(
                      `https://fastapi-data-1-nc7j.onrender.com/transactions/${transactionId}/itinerario/${(existingFlight as any).id}`,
                      {
              ...
              const requiresHotel = ["nacional", "internacional", "terrestre"].includes(formData.package);
              if (requiresHotel && formData.hotelInfo && formData.hotelInfo.length > 0) {
              ...
              console.log("✅ Estado actualizado a 'pending':", statusResponse.data);
              
              // Refrescar los datos para que se actualice la vista
              console.log("🔄 Refrescando datos...");
              await refreshTransactions();
              
              alert("✅ Información completada y enviada a confirmación exitosamente");
              ...
              if (error.response) {
                ...
                alert(`❌ Error al enviar a confirmación: ${error.response.data?.detail || error.message}`);
              } else {
